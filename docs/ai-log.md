...
| 2026-07-09 | v1         | 修正 GitHub Pages `/v1/` 直連          | 在 build 後複製 `dist/index.html` 到 `dist/v1/index.html`，讓 Pages 靜態站能直接開 `/v1/`                          | 採用；不改成 hash route                      | 課程規則要求 v1 從 `/v1/` 存取；這只是靜態輸出 fallback，不新增後端、資料庫或外部 API                  | `package.json`, `scripts/prepare-pages-routes.ts`, `docs/ai-log.md`                                                                     |
...
| 2026-07-09 | Release 02 | 依需求取捨產生 v1 流程 Mermaid         | 根據 `docs/decisions.md` 建議把流程聚焦在回報者留下可追溯觀測、疑似相關只提示不合併、缺漏與可行動風險送人工確認    | 採用；不修改前端程式                         | Release 02 是流程設計階段；流程必須避免把未確認資訊升級成 confirmed / verified，也不能讓 AI 做派工判斷 | `docs/flow.md`, `docs/ai-log.md`                                                                                                        |
| 2026-07-09 | Release 01 | 用 sub-agent 重新評估目前 v1           | 三個 persona 指出「現場補充數」「親眼看到」「我在現場」「非常急迫」可能被誤讀成可靠度或行動優先順序                | 採用為評估草稿；拒絕轉成行動判斷             | 這是 persona sub-agent 模擬，不是真實訪談；v1 仍服務回報者與整理前材料，不提供 confirmed 或出發判斷    | `docs/interview-notes.md`, `docs/interview-summary.md`, `docs/decisions.md`, `docs/ai-log.md`                                           |
...
| 2026-07-09 | v1         | 將統一預設觀測文字改成示範假資料       | 建議依 M-001 到 M-012 提供不同的假資料欄位與說明，並在風險區標示示範假資料不能作為行動依據                         | 採用；不改查核狀態、不新增 fixture           | 假資料只用於前端示範與掃描，不代表人工確認完成，也不能替代原始資訊或救災判斷                           | `src/features/v1/v1-mappers.ts`, `docs/ai-log.md`                                                                                       |
| 2026-07-09 | v1         | 將兩筆改成組織演練可行動案例           | 使用 M-009、M-010 加入 `field_checked_actionable` 狀態，顯示組織實地核查完成與內部可行動條件                       | 採用；限定為演練假資料                       | 這不代表 Phase 0 原始資訊被真實查核；只是示範若組織有人實地核查後，工作台如何顯示可行動案例            | `src/features/v1/v1-types.ts`, `src/components/status-labels.ts`, `src/features/v1/v1-mappers.ts`, `src/features/v1/V1Workbench.tsx`    |
